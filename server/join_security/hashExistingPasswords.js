...
const updatePasswords = async () => {
    ...
    try {
        ...
        const users = await conn.query('SELECT user_id, user_password FROM tb_user');

        for (let user of users) {
            // 비밀번호가 이미 해시된 상태인지 확인 (bcrypt 해시의 길이는 60)
            if (user.user_password.length !== 60) {
                ...
                // 데이터베이스에 업데이트
                await conn.query('UPDATE tb_user SET user_password = ? WHERE user_id = ?', [hashedPassword, user.user_id]);
