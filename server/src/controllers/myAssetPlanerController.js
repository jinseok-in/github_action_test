...
const getUserCapital = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_capital FROM tb_user WHERE user_id = 1 LIMIT 1');
...
const getUserTargetBudget = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_target_budget FROM tb_user WHERE user_id = 1 LIMIT 1');
...
const updateUserTargetBudget = async (req, res) => {
  ...
  try {
    const result = await pool.query('UPDATE tb_user SET user_target_budget = ? WHERE user_id = 1', [targetBudget]);
...
const getUserDeposit = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_deposit FROM tb_user WHERE user_id = 1 LIMIT 1');
    console.log(result); // 쿼리 결과를 출력해 확인
    if (result.length === 0) {
      return res.status(404).json({ error: 'User not found' });
    }
...
const getUserInstallmentSaving = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_installment_saving FROM tb_user WHERE user_id = 1 LIMIT 1');
    console.log(result); // 쿼리 결과를 출력해 확인
    if (result.length === 0) {
      return res.status(404).json({ error: 'User not found' });
    }
...
const getUserLoan = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_loan FROM tb_user WHERE user_id = 1 LIMIT 1');
    console.log(result); // 쿼리 결과를 출력해 확인
    if (result.length === 0) {
      return res.status(404).json({ error: 'User not found' });
    }
